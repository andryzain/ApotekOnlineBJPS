    ...
    public class UserActiveController : Controller
    {
        ...
        public async Task<IActionResult> AddUser([FromBody] UserActiveViewModel userActive)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var checkDuplicate = _applicationDbContext.UserActives.Where(c => c.UserActiveCode == userActive.UserActiveCode && c.FullName == userActive.FullName).ToList();

                if (checkDuplicate.Count == 0)
                {
                    var result = _applicationDbContext.UserActives.Where(c => c.UserActiveCode == userActive.UserActiveCode && c.FullName == userActive.FullName).FirstOrDefault();
                    if (result == null)
                    {
